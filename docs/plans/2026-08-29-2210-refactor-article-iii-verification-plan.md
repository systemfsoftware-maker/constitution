...
- **Objective:** The constitution's verification law binds only what measured evidence supports — oracle independence, observer-fit test placement, change-relevant mutation over a named non-empty set, published-contract pinning before deletion, and recomputed gate keys — and no longer prescribes the Testing Trophy, mandatory mocked composition, blanket property coverage of the core, or characterization as a standing duty. A reader outside this repo can confirm it: `pnpm test` passes with 33 rules across both corpus files, `pnpm test --against <rev>` run after the corpus commit names exactly CONST-T1, CONST-T2, and CONST-T5 as vacated with no id reassigned, and no corpus text names a trophy or a characterization duty.
- **Means:** A validator reporting fix, then rule surgery on Article III (vacate T1/T2/T5, reword T3, keep T4, mint T8–T14 plus CONST-E5), then the S4 repoint and harness-prose reconciliation (KTD1–KTD5; normative text in Appendix A and B).
- **Authority:** The H1–H12 verdict dossier and the org-mechanism specification (both delivered in the invoking conversation) govern doctrine conflicts. The corpus's own id-surgery table in AGENTS.md governs id handling. The current corpus at `origin/main` (28 rules: 7 resident Article V + 21 retrieved) is the base being amended, not evidence of correctness (CONST-S2).
- **Execution profile:** One feature branch, three conventional commits (validator, corpus, harness prose) pushed together in one PR; no consumer-repo changes.
- **Stop conditions:** A validator failure not explained by the amendment's own edits stops the run — a defect in the amendment's YAML gets fixed, a defect in the gate gets reported, never patched around. An edit whose find-pattern misses (grep still finds the old text after the edit) stops the run for amendment repair, not a patch-around. Implementation-time evidence contradicting a settled decision stops the run.
- **Tail ownership:** Branch shipping, PR, and CI watch belong to the calling pipeline. The consumer repo's watchdog and mutation-config updates belong to that repo after its next vendor pull.
...
- **Vacate CONST-T1 and CONST-T5 rather than reword them.** The trophy and the characterization duty are refuted doctrine, not rewordable ones; their numbers stay vacant forever per the corpus's own id-surgery rule. (session-settled: user-directed — chosen over keeping trophy and characterization as reworded doctrine: the org tree's measured mechanisms and the dossier refuted both.) Governs R1, R2, R19.
- **The dossier adjudicates doctrine conflicts.** Where the org tree's current mechanisms and the evidence disagree, the evidence wins: mutation scope is change-relevant, not decision-only (H5); the organizing principle is oracle independence, not a surface count (H2). (session-settled: user-directed — chosen over encoding the org repo's mechanisms verbatim: H5 refuted decision-only scoping as observer-fit and H2 refuted the exhaustive two-surface claim.) Governs R4, R7.
- **CONST-E5 ships in this amendment.** The unforgeable-gate rule is in scope now, not deferred. (session-settled: user-directed — chosen over deferring gate-verification doctrine to a later amendment: user directed inclusion when the deferral was presented as default.) Governs R13.
- **Principles only — no tool names, no consumer names, no filename suffixes in rule YAML.** Mutation is an instrument class; how a tree scopes its mutant set is that tree's config. (session-settled: user-directed — chosen over naming the enforcing stack as exemplar: consumer repos in other languages vendor this file, and a named first consumer turns a house rule into law.) Governs R18.

### Requirements

**Validator reporting**

- R1. `pnpm test --against <rev>` names, on its success line, every id vacated since `<rev>` and every corpus file absent at `<rev>`, per the output shape AGENTS.md and `docs/solutions/architecture-patterns/the-vacuous-pass-gate-input-sets.md` already specify; the change touches reporting only — schema fields, family registry, and gate registration are untouched.
...
- R16. The validator's module docstring (`scripts/validate-constitution.py`) cites CONST-T9 in its union example instead of CONST-T5; no other docstring change rides with the corpus commit (the reporting change is R1 and ships in its own commit).
- R17. AGENTS.md's file-choice paragraph cites CONST-T14 as its review-gated artifact-announced testing example instead of CONST-T2; the "28 of 34 rules" count sentence is recomputed against the amended corpus (33 rules) and restated with measured numbers; its vacated-id reporting claim is now backed by R1's validator behavior.
- R18. README.md's Article III row describes the new doctrine without naming the Testing Trophy, and the "Rules: 34" badge is recomputed to 33.
...
- A probe that adds a citation to a vacated id makes `pnpm test` fail naming the dangling citation; the probe is reverted after the demonstration.
...
In scope: `CONSTITUTION-ARTICLES.md`, `CONSTITUTION.md`, `scripts/validate-constitution.py` (reporting output plus docstring), `AGENTS.md`, `README.md`, and this plan.
...
- KTD5. **Three commits, pushed together as one PR: validator, corpus, harness prose.** The validator reporting change (R1) is an evaluator change and lands first, in its own commit, so the corpus commit it judges is verified by a gate that already reports vacancies. The corpus commit carries both constitution files. The harness prose (docstring, AGENTS.md, README.md) lands third. The intermediate states exist only on the local branch for minutes — CI and consumer repos see only the PR head — so the corpus/prose divergence window never publishes. This split keeps each layer auditable on its own without mixing an evaluator change with the work it judges.
...
- Consumer repos citing CONST-T1, CONST-T2, or CONST-T5 fail loudly at their next vendor pull. That is the intended vacancy behavior — a citation into a gap resolves to nothing, which the validator surfaces. One consumer is known to cite them (the org tree's `WATCHDOG.md`, per the invoking conversation); its repair is the deferred follow-up, not a shim here.
...
Every consumer repo's agents receive the amended doctrine at their next vendor pull; the constitution repo's own harness (AGENTS.md guidance, README's doctrine table and badge) changes with it. No production code changes anywhere. The validator's checking behavior is unchanged — same schema, families, and gates — with one reporting addition (R1) that makes its own documented output true.

---

## Implementation Units

### U5. Validator `--against` vacancy reporting

**Goal:** `pnpm test --against <rev>` names every id vacated since the revision and every corpus file absent there, on the success line — the output AGENTS.md and the vacuous-pass learning already specify.

**Requirements:** R1

**Dependencies:** none (executes first)

**Files:** `scripts/validate-constitution.py`

**Approach:**

1. Read the current `check_against` before editing; the function builds `old_titles` from `git show <rev>:<path>` and errors only on surviving ids whose title changed.
2. Add to the `--against` success path: ids present at `<rev>` but absent from the live corpus are listed as vacated; corpus paths absent at `<rev>` are listed as not compared. Both print on the success line; neither fails the run (a deliberate deletion with no surviving citation is legal).
3. Check behaviors, not fields: schema, family registry, and gate validation are untouched.

**Patterns to follow:** the reporting shape in `docs/solutions/architecture-patterns/the-vacuous-pass-gate-input-sets.md` ("what cannot be failed must be reported").

**Test scenarios:**

- Against a pre-#9 revision: names exactly CONST-E1, CONST-E2, CONST-E3, CONST-E4, CONST-G1, CONST-G2 as vacated; exit 0.
- Against the current HEAD: names nothing vacated; exit 0.
- Against a revision predating one corpus file: names that file as not compared; exit 0.

**Verification:** the three scenarios pass with readable failure-free output; `pnpm test` without `--against` is unchanged.
...
- Dangling-citation probe: temporarily add a citation to CONST-T5 in a corpus file; `pnpm test` fails naming the dangling citation; revert the probe and confirm green.
...
**Dependencies:** U1, U2, U5 (prose must match final rule text, ids, and validator behavior)

**Files:** `scripts/validate-constitution.py`, `AGENTS.md`, `README.md`

**Approach:**

1. In the validator's module docstring, change the union example's "CONST-S4 cites CONST-T5" to cite CONST-T9. No code changes.
2. In AGENTS.md's file-choice paragraph, replace the CONST-T2 example citation with CONST-T14.
3. In AGENTS.md's gate-explanation paragraph, recompute the inbound-citation count and the corpus total (33) against the amended files and restate the sentence with the measured numbers; its vacated-reporting claim now names real behavior.
4. In README.md's doctrine table, rewrite the Article III row to describe observer-fit placement, properties by narrow grant, and mutation as the measure; recompute the "Rules" badge from 34 to 33.

**Patterns to follow:** AGENTS.md's existing measured-claim style; README.md's table-row and badge shapes.

**Test scenarios:**

- `pnpm test` exits 0 after the docstring edit.
...
**Verification:** the three scenarios pass.

### U4. Verification battery and commits

**Goal:** Evidence is recorded and the work lands as three conventional commits.
...
2. Commit the validator change (U5) first with a `fix(scripts):` message; then the corpus change (U1 + U2) with a breaking-change `refactor(constitution)!:` message; then the harness prose (U3) with a `docs:` message.
3. Run commitlint over all three commits.

**Test scenarios:**

- All three commits pass `pnpm exec commitlint --from HEAD~3`.
...
| Commit format | `pnpm exec commitlint --from HEAD~3` | Exit 0 for all three commits |
...
- The three commits (validator, corpus, harness prose) are separate, conventional, and pushed together in one PR.
...
- `scripts/validate-constitution.py`: gate behavior read in full — schema fields, family registry, cross-file citation resolution, title-keyed `--against` reassignment check over ids present at both revisions; vacancy reporting is absent today and is added by U5 of this plan per the documented design.
- AGENTS.md: id-surgery table (obligation identity), family registry, file-choice rule, the gate-explanation paragraph whose counts and vacated-reporting claim R17 recomputes and now verifies.
- `docs/solutions/architecture-patterns/the-vacuous-pass-gate-input-sets.md`: local grounding for CONST-E5 and the `--against` reporting shape — a gate keyed on author-supplied values certifies nothing; what cannot be failed must be reported.
